...
It caches results to prevent duplicate fetch/parse work and returns a promise for convenient
usage in async Svelte components.
...
<script>
  ...
  const gltf = await useGltf('/assets/model.gltf')
</script>
...
<script>
  ...
  const texture = await useTexture('/assets/texture.png')
</script>
...
The return value from [useLoader](/docs/reference/core/use-loader) is a `Promise`
that will be unresolved until the asset has loaded.

```svelte
<script>
  import { useTexture } from '@threlte/extras'

  const map = await useTexture('/path/to/texture.png')
</script>
