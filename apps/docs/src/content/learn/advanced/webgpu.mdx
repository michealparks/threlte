...
**\<Canvas>**'s _createRenderer_ prop. This will replace the default WebGL renderer.
...
The **\<T>** component uses all the exports from _three_ thus it will error on things
like **\<T.MeshPhysicalNodeMaterial />** because the **MeshPhysicalNodeMaterial**
class is an export of _three/webgpu_ and not _three_. You have a few options to
work this out.

1. Extend **\<T>** with all the definitions from _three/webgpu_ by using the
   ...
   your application because both _three_ and _three/webgpu_ will be imported in
...
2. Use explicit imports for the objects, functions, and other classes that you
   use from **three/webgpu**. You can then use **\<T>**'s
   [_is_](/docs/reference/core/t#property-is) prop with those imports from
   _three/webgpu_.
...
3. Same as option #2 but using
   [_extend_](/docs/reference/core/t#extending-the-default-component-catalogue)
...
#### Careful! _three_ and _three/webgpu_ don't mix well

You will need to overwrite some of the default catalogue if you use
_three/webgpu_. For example, if you're using a **MeshPhysicalNodeMaterial**, you
...
This is because the exports from _three/webgpu_ are different than those in
_three_ and make use of the additional features that node materials have.
...
The
material's [nodes](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language#nodematerial)
...
Or can create the material in the script tag and use **\<T>**'s _is_ prop to attach the material.
...
in the **uColor** uniform. The material needs its _side_ set to
