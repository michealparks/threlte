...
The return type of `load` is a promise and can be awaited
in async Svelte components.
...
<script>
  ...
  const map = await load('path/to/texture.png')
</script>
