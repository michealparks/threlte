...
A hook to load data with an arbitrary `THREE` loader class. The raw loader
result is cached and subsequent calls with the same arguments will reuse the
same loaded asset across the entire Threlte app. The hook can use async loaders
(using `loader.loadAsync`) as well as sync/callback-based loaders (using
`loader.load`) and will default to the async version if available.
...
The raw loaded asset is cached before `transform` runs, so each call to `load` applies its own
transformation to the cached asset.
