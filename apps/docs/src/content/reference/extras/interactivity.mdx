...
## Refreshing pointer state

By default, the interactivity plugin only raycasts in response to DOM pointer events. If your
scene has animated objects that move under a stationary cursor, hover state will go stale until
the user moves the pointer. You can call `refreshPointerState()` to force a re-evaluation of
hover state using the last known pointer position.
...
<script>
  ...
  const { refreshPointerState } = useInteractivity()

  // Re-evaluate hover state every frame (useful when objects animate under the cursor)
  useTask(() => {
    refreshPointerState()
  })
</script>
