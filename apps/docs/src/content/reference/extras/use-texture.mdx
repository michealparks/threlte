...
<Tip type="warning">
  Be aware that the loaded texture is cached by URL. Each call applies its own `transform` to that
  cached texture unless your transform returns a separate object.
</Tip>
