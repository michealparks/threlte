...
export const useFBO = (options?: () => UseFBOOptions | undefined): WebGLRenderTarget => {
  ...
  const { size: _size, depth: _depth, ...initialTargetOptions } = initial
  const target = new WebGLRenderTarget(1, 1, initialTargetOptions)
  ...
  $effect(() => {
    ...
    } else if (depth !== false && depth !== undefined) {
